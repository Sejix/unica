...
- Preferred path: use MCP `unica` tools `unica.code.search`, `unica.code.diagnostics`, `unica.project.map`, `unica.subsystem.info`, `unica.meta.info`, `unica.standards.search`, `unica.standards.explain`, and `unica.runtime.execute`.
...
2. Find the candidate API and callers with `unica.code.search`; include export area comments, module suffixes, and deprecated sections in the evidence.
